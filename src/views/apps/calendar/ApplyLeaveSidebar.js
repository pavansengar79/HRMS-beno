...
const ApplyLeaveSidebar = props => {
  ...
  const {
    ...
    deleteEvent,
    handleSelectEvent,
    applyLeaveSidebarOpen,
    handleApplyLeaveSidebarToggle
  } = props
  ...
  const onSubmit = data => {
    const employee = 'Current User'
    const leaveType = values.calendar
    const manager = values.guests[0] || 'Unknown'
    const modifiedEvent = {
      url: '',
      display: 'block',
      title: `Leave Application: ${employee} - ${leaveType}`,
      end: values.endDate,
  ...
  }, [applyLeaveSidebarOpen, resetToStoredValues, resetToEmptyValues, store.selectedEvent])
    ...
    >
      ...
      >
        <Typography variant='h5'>
          {store.selectedEvent !== null && store.selectedEvent.title.length ? 'Update Leave Application' : 'Apply Leave'}
        </Typography>
...
export default ApplyLeaveSidebar
