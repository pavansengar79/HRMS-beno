...
import Box from '@mui/material/Box'
import Modal from '@mui/material/Modal'
import { fetchrepairData, updateTyre } from 'src/store/apps/roiMasters'
...
const EditModal = ({ data, onClose }) => {
  ...
  const [name, setName] = useState(data.Ydesc)
  ...
  const handleSubmit = e => {
    e.preventDefault()
    if (name && price && margin) {
      dispatch(updateTyre({ id: data._id, avgPrice: price, name: name, margin: margin }))
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <Grid container justifyContent='center' columnGap={4} sx={{ mt: 4 }}>
