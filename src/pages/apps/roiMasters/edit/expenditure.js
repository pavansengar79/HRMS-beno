...
import Box from '@mui/material/Box'
import Modal from '@mui/material/Modal'
import { fetchrepairData, updateExpenditure } from 'src/store/apps/roiMasters'
...
const EditModal = ({ data, onClose }) => {
  // ** States
  const [name, setName] = useState(data?.category)
  const [date, setDate] = useState(null)
  const [language, setLanguage] = useState([])
  ...
  const handleSubmit = e => {
    e.preventDefault()
    if (name) {
      dispatch(updateExpenditure({ id: data._id, category: name }))
