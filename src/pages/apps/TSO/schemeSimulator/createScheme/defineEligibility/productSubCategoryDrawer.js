  ...
  useEffect(() => {
    ...
    if (selectedCategory?.includes('Non Truck')) {
      options.push(
        ...
        { value: 'R1', label: 'TREEL' },
        { value: 'F1', label: 'KARTING TYRES' }
      )
    }
    if (selectedCategory?.includes('Truck')) {
      options.push({ value: '11', label: 'TRUCK BIAS' }, { value: '12', label: 'TRUCK RADIAL' })
    }
    if (selectedCategory?.includes('OTR')) {
      options.push({ value: '81', label: 'OTR BIAS' }, { value: '82', label: 'OTR RADIAL' })
    }
    if (selectedCategory?.includes('JK Treel')) {
      options.push({ value: 'R1', label: 'TREEL' })
    }
    if (selectedCategory?.includes('Retread')) {
      options.push(
        ...
        { value: 'VUL', label: 'VULCONIZED' },
        { value: 'CU', label: 'CU GUM' }
      )
    }
