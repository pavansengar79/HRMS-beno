...
import { fetchrepairData, changerepairStatus, updateRejectData } from 'src/store/apps/repair-reason'
...
const Repair = () => {
  ...
  const data = useSelector(state => state.repair)

  const files = data?.repair?.map((n, i) => {
    return {
      ...n,
  ...
  useEffect(() => {
    dispatch(fetchrepairData({ paginationModel, search: search }))
  ...
  return (
    <Grid item xs={12}>
      <Card>
        ...
        />
        {modalType === 'add' ? (
          <CustomDialog show={show} setShow={setShow} title='Add Repair Reasons'>
            <AddModal onClose={closeModal} onUpdate={updateValue} />
          </CustomDialog>
        ) : (
          <CustomDialog show={show} setShow={setShow} title='Edit Repair Reasons'>
