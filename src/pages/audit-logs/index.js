...
const AuditLogsPage = () => {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      if (res?.success) {
        // Transform data: map _id to id for DataGrid
        const transformedLogs = (res.data.logs || []).map(log => ({
          ...log,
          id: log._id || log.id
        }))
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: ({ row }) => {
        ...
        const companyName = row.company_id?.company_name || row.company_id?.brand_name
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    ...
                    {(selectedLog.company_id?.company_name || selectedLog.company_id?.brand_name) && (
                      <Chip size='small' label={`Company: ${selectedLog.company_id?.company_name || selectedLog.company_id?.brand_name}`} color='secondary' />
                    )}
